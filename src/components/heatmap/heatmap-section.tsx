...
type HeatmapSectionProps = {
  /** Live sectors fetched at request time; empty falls back to mock data. */
  liveSectors: Sector[];
...
export function HeatmapSection({
  ...
  const isLive = liveSectors.length > 0;
  ...
  const hasHorizonData = useMemo(
    () =>
      liveSectors.some(
        (s) =>
          typeof s.changePct1w === "number" ||
          typeof s.changePct1m === "number",
      ),
    [liveSectors],
  );
  ...
  const sectors = useMemo(() => {
    const list =
      market === "KR"
        ? isLive
          ? liveSectors.filter((s) => s.market === "KR")
          : MOCK_KR
        : market === "US"
          ? isLive
            ? liveSectors.filter((s) => s.market === "US")
            : MOCK_US
          : isLive
            ? liveSectors
            : MOCK_ALL;
  ...
  }, [market, horizon, liveSectors, isLive]);
    ...
    <div className="flex flex-col gap-6 md:gap-8">
      <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm">
        ...
        <span className="text-muted-foreground">
          {isLive
            ? `1시간마다 자동 갱신${fetchedAt ? ` · 마지막 ${formatFetchedAt(fetchedAt)}` : ""}`
      ...
      <div className="flex flex-col gap-4">
        ...
        {isLive && !hasHorizonData && horizon !== "1d" && (
